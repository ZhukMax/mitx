...
class Patient(object):
    ...
    def getTotalPop(self):
        ...
        """
        if self.maxPop > len(self.viruses):
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.getViruses():
            try:
                new_virus.append(virus.reproduce(popDensity, self.prescriptions))
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for j in range(150):
            ...
            virus_count.append(patient.getTotalPop())
            resist_virus_count.append(patient.getResistPop(['guttagonol']))
        data_1 = data_1 + virus_count
        data_2 = data_2 + resist_virus_count
    data1_avg = data_1 / numTrials
    data2_avg = data_2 / numTrials

    pylab.plot(list([float('{0:.1f}'.format(i)) for i in data1_avg]), label='Non-resistant population')
    pylab.plot(list([float('{0:.1f}'.format(j)) for j in data2_avg]), label='Guttagonol Resistant population')
